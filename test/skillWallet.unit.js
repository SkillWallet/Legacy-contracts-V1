...
contract("SkillWallet", function () {
  beforeEach(async function () {
    [creator, communitySign, skillWalletOwner, ...addrs] =
    ...
    const OffchainSignatureMechanism = await ethers.getContractFactory(
      "OffchainSignatureMechanism"
    );
    ...
    await linkTokenMock.transfer(osmAddress, "2000000000000000000");
  });
  ...
  describe.skip("SkillWallet", async function () {
    describe("Creating a SkillWallet", async function () {
      it("should fail when there is already a SW to be claimed by this user", async function () {
        console.log(skillWalletOwner.owner);
        const tx = await (
        ...
        const SWCreated = tx.events.find(
      ...
      it("should fail when the user already owns a SW", async function () {
        ...
        const claimedTx = await (
          await skillWallet.connect(skillWalletOwner).claim()
        ).wait();
        const claimedTxEvent = claimedTx.events.find(
      ...
      it("should create a claimable inactive SW", async function () {
        const tx = await (
          await skillWallet
            .connect(communitySign)
            .create(skillWalletOwner.address, metadataUrl, 1, true)
        ).wait();
        ...
        const skillWalletCommunityHistory =
          await skillWallet.getCommunityHistory(tokenId);
      ...
      it("should create an inactive SW", async function () {
        const tx = await (
          await skillWallet
            .connect(communitySign)
            .create(skillWalletOwner.address, metadataUrl, 1, false)
        ).wait();
      ...
      it("should claim SW and transfer the token to the owner", async function () {
        ...
        const owner = await skillWallet.ownerOf(tokenId);
    ...
    describe("Adding a pubKey to a SkillWallet", async function () {
      it("should fail when the SW is not created yet", async function () {
        const failingTx = skillWallet
          .connect(creator)
          .addPubKeyToSkillWallet(1000000, "");
        await truffleAssert.reverts(
          failingTx,
      ...
      it("should fail when the call is not made by the owner", async function () {
        const failingTx = skillWallet
          .connect(skillWalletOwner)
          .addPubKeyToSkillWallet(1, "");
        await truffleAssert.reverts(
          failingTx,
      ...
      it("should fail when the SW has not been claimed yet.", async function () {
        ...
        const failingTx = skillWallet
          .connect(creator)
          .addPubKeyToSkillWallet(tokenId, "pubKey");
      ...
      it("should fail when the SW has pubKey already assigned.", async function () {
        ...
        const pubKeyTx = await (
          await skillWallet
            .connect(creator)
            .addPubKeyToSkillWallet(tokenId, "pubKey")
        ).wait();

        const pubKeyEventEmitted = pubKeyTx.events.find(
          (e) => e.event == "SkillWalletCreated"
        );
    ...
    describe("Activate skillWallet", async function () {
      ...
      it("should activate the SW if the response of the chainlink callback is true", async function () {
        const signature = "";
        const validationTx = await (
          await osm.validate(signature, tokenId, 0, [], [], [])
        ).wait();

        const validationRequestIdSentEventEmitted = validationTx.events.find(
        ...
        const fulfilTx = await (
          await mockOracle["fulfillOracleRequest(bytes32,bool)"](
            requestId,
            true
          )
        ).wait();

        const fulfilTxEventEmitted = fulfilTx.events.find(
          (e) => e.event == "CallbackCalled"
        );

        assert.isNotNull(fulfilTxEventEmitted);

        const isSWActivated = await skillWallet.isSkillWalletActivated(tokenId);
