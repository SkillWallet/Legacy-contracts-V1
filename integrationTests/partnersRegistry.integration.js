const partnersRegistryAddress = "0x8241C45fA0211a71d3693317e93628C3F8FfB98c";
...
var partnersRegistryAbi =
  require("../artifacts/contracts/main/partnersAgreement/contracts/PartnersRegistry.sol/PartnersRegistry.json").abi;
...
var communityRegistryAbi =
  require("../artifacts/contracts/main/community/CommunityRegistry.sol/CommunityRegistry.json").abi;
...
async function getPAs() {
  const createTx =
    await partnersRegistryContract.getPartnerAgreementAddresses();
  console.log(createTx);
}
async function createCommunity() {
  const a = await (
    await communityRegistryContract.createCommunity(
      url,
      1,
      100,
      10,
...
async function createPartnersAgreement(comAddr) {
  ...
  console.log(
    "PartnersAgreement: ",
    events[0].args["partnersAgreementAddress"]
  );
  return events[0].args["partnersAgreementAddress"];
}

async function setPartnersAgreementFactory(factory) {
  const createTx = await partnersRegistryContract.setPAFactory(factory);
}
...
async function addCoreTeamMember(communityAddress, memberAddress) {
  ...
  const joinedTx = await communityContract.addNewCoreTeamMembers(memberAddress);
  const joinCommunityTxResult = await joinedTx.wait();
  const { events } = joinCommunityTxResult;
  const memberAddedEventEmitted = events.find(
    (e) => e.event === "CoreTeamMemberAdded"
  );
...
async function getActivities(partnersAgreementAddr) {
  ...
  console.log(
    ...
    await partnersAgreementContract.getActivitiesAddress()
  );
  console.log(
    "interactionNFT",
    await partnersAgreementContract.interactionNFT()
  );
}
...
async function findByOwner(owner) {
  const aggrr = await partnersRegistryContract.getPartnerAgreementAddresses();

    for (let i = 0; i < aggrr.length; i++) {
      const partnersAgreementContract = new ethers.Contract(
        aggrr[i],
        partnersAgreementAbi,
        signer
      );
      const paOwner = await partnersAgreementContract.owner()
      if(paOwner == owner) {
          console.log('partnersAgreement', aggrr[i]);
          console.log('communityAddress', await partnersAgreementContract.communityAddress());
      }
    }
  // const communityContract = new ethers.Contract(
  //   "0xa31C9eaDDC2cf50a7a139b891efA302F040AD100",
  //   communityAbi,
  //   signer
  // );

  // const a = await communityContract.owner();
  // console.log(a);
}
...
async function test() {
  ...
  await findByOwner("0x463D93b4d864fa1D9F70DaC4876Ca55d2a310543");
